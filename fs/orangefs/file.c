			   ...
			   size_t total_size)
{
	...
	if (total_size) {
		...
		ret = pvfs_bufmap_copy_from_iovec(bufmap,
						...
						total_size);
		if (ret < 0)
		gossip_err("%s: Failed to copy-in buffers. Please make sure that the orangefs-client is running. %ld\n",
			   __func__,
			   (long)ret);
	}
			    ...
			    size_t total_size)
{
	...
	if (total_size) {
		...
		ret = pvfs_bufmap_copy_to_iovec(bufmap,
						...
						buffer_index);
		if (ret < 0)
			gossip_err("%s: Failed to copy-out buffers. Please make sure that the orangefs-client is running (%ld)\n",
				__func__,
				(long)ret);
	}
	return ret;
